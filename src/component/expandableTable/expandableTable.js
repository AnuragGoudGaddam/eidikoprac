...
const ExpandableTableRow = ({ row, onEdit, onDelete }) => {
    const handleEdit = (event, column) => {
      onEdit(row.id, event.target.value, column);
    };
...
const ExpandTable = () => {
    ...
    const handleAddRow = () => {
        ...
        const combinedString = tableData.map(row => `${row.column1}=${row.column2}`).join(', ');
    ...
    return (
      <div className="App_jss">
        <div>
          <table className="expandable-table" style={{ border: '' }}>
            <thead>
              <tr>
                <th className="small-cell">service Code</th>
                <th className="small-cell">Amount</th>
